...
const tMain = createI18n({
  ...
  cs: {
    ...
    errGskNotLoggedIn:
      'Nejste přihlášeni ke Friday: klikněte níže na „Přihlásit se ke Friday“, přihlaste se a zkuste to znovu',
...
export function registerSheetsAiIpc(): void {
  ...
  // Friday account (Friday state): the auth source for AI features; the
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    ...
    const provider = request.settings.provider as AiProviderId
    let config = request.settings.providers[provider]
    // Friday's key never enters the settings file; it is read from the gsk
...
async function applyMainProcessProxy(): Promise<void> {
  ...
  try {
    ...
    // Friday LLM proxy and the gsk CLI actually target
