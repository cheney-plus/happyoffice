...
export interface DeckAccess {
  ...
  /** Whether cloud single-page generation is available (kill switch + Friday state) */
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: {
          type: 'string',
          description:
            'Optional, defaults to the configured model. Friday only — specify for special purposes: fal-bria-rmbg=background removal, fal-ai/recraft-clarity-upscale=upscale, flux-pro/outpaint=outpaint, fal-ai/image-editing/text-removal=remove text watermark',
        },
        referenceImageUrls: {
          type: 'array',
          items: { type: 'string' },
...
/** tools that need a media provider: Friday login + cloud tools, or a BYOK media key in Settings */
...
function mediaToolsOffNote(hidden: Set<string>): string {
  ...
  return `\n\nNote: ${[...hidden].join(' and ')} ${plural ? 'are' : 'is'} currently unavailable (no image/media provider: signed out of Friday or cloud tools off, and no media API key in Settings). Do not call or promise ${plural ? 'them' : 'it'}; for imagery use image_search + insert_web_image instead.`
}
